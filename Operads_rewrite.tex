...
\Def{presentation} of an operad $\Oca$ generated by one binary generator:
it is the data of a generating set of an operad congruence $\Congr$ of
$\Mag$ such that $\Oca$ is isomorphic to
\begin{math}
  \Mag/_{\Congr}.
\end{math}
A presentation is said to be \Def{finite} if it is a finite set. For
simplicity, we say that a convergent (resp. terminating, confluent)
orientation of $\Congr$ is a \Def{convergent} (resp. \Def{terminating},
\Def{confluent}) \Def{presentation} of $\Oca$. 
\medbreak
...
\begin{enumerate}[label={(\it\arabic*)}]
  ...
  \item\label{item:choice_branching_pair} Let $\{\Sfr_1, \Sfr_2\}$ be a
    branching pair for $\Tfr$.
    \smallbreak

  \item\label{item:computed_normal_forms} Let $\overline{\Sfr_1}$ and
    $\overline{\Sfr_2}$ be normal forms of $\Sfr_1$ and $\Sfr_2$,
    respectively.
    \smallbreak
...
and~\ref{item:choice_branching_pair} as well as the computed normal forms
at step~\ref{item:computed_normal_forms}.
\medbreak
