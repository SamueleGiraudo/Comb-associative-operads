...
\Todo{Christophe : je vous propose cette démo}
\begin{proof}
    ...
    \begin{equation} \label{eq:rewriting_invariant}
        ...
        \, \pmod{\bar{\gamma}}.
    \end{equation}
    Indeed for all $\Tfr \in \Mag$, there exist binary trees 
    $\Tfr_1, \dots, \Tfr_{\LRank\left(\Tfr\right)}$ such that
    \begin{math}
      \Tfr = 
      \left[\, |,\Tfr_1, \dots, \Tfr_{\LRank\left(\Tfr\right)} \right].
    \end{math}
    Thus if $\Tfr \RewContext \Tfr'$, then one of the 
    two following cases occurs :
    \begin{enumerate}[label={(\it\roman*)}]
        \item there exist $\Tfr_i'$ such that
            \begin{math}
        ...
        \item there exist $i$ such that
            ...
            \end{math}.
    \end{enumerate} 
    This concludes the proof of~\ref{eq:rewriting_invariant}.  
    \smallbreak
    
    We deduce from~\ref{eq:rewriting_invariant} that the equivalence 
    ...
    \begin{equation}
        ...
        \, \pmod{\bar{\gamma}}
    \end{equation}
    contains the relation $\RewContext$ and thus its transitive 
    and symmetric closure, which is $\CongrCAs{\gamma}$.
\end{proof}
...
Note that $\LatKCAs$ does not embed as a sub-lattice of $\LatQMag$, that
is $\iota$ is not a lattice morphism. Consider for instance $\gamma = 3$
and $\gamma' = 4$, then $\KCAs{\gamma} \InfCAs \KCAs{\gamma'}$ is equal
to $\KCAs{2} = \K\As$, whereas $\KCAs{\gamma} \InfQMag \KCAs{\gamma'}$
is a quotient of $\KMag$ be the two relations $\LComb{3} - \RComb{3}$
and $\LComb{4} - \RComb{4}$.
\medbreak
...
and $7$ provides the new relations \smallbreak
...
\Todo{S: résultats expérimentaux de quelques $\CAs{\gamma}$ à donner
dans cette partie (ou peut être dans une sous-partie précédente ; à
voir selon ce que ça donne).}

%By computer exploration, we have already some information in this
%direction: we have the sequence
%\begin{equation}
