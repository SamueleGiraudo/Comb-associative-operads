...
Recall that $\Oca_1$ and $\Oca_2$ are \Def{anti-isomorphic} if there
exists a map (called \Def{anti-isomorphism})
...
\begin{Proposition} \label{prop:Recall_anti_isomorphic}
    ...
    \item if $\{ x_i \rightarrow y_i \}_{1 \leq i \leq n}$ is a convergent
     ...
     \phi\left(y_i\right) \}_{1 \leq i \leq n}$ is a convergent orientation
      of $\Oca_2$. \Todo{Samuele: je ne sais pas si cela respecte les
      notations et conventions de la partie 1}
...
\end{split}
\end{equation}
\Todo{S : il faudrait utiliser les memes conventions que dans la partie
3 pour représenter les morceaux fait de $k$ occurrences d'un noeud
(voir l'eq (3.2.3)}
