...
Let us now define for any $d \geq 1$ the \Def{$d$-comb associative
operad} $\CAs{d}$ as the quotient operad $\Mag/_{\CongrCAs{d}}$ where
$\CongrCAs{d}$ is the smallest operad congruence of $\Mag$
satisfying
\begin{equation} \label{equ:congruence_Asd}
    \underbrace{(\dots (\Product \circ_1 \Product) \circ_1 \dots)
        \circ_1 \Product}
    _{d \mbox{ \footnotesize operands}}
    \enspace \CongrCAs{d} \enspace
...
comb binary trees of degree $d$ are equivalent for $\CongrCAs{d}$.
...
In the sequel we denote by $\LC{d}$ and $\RC{d}$ the left comb and the
right comb of degree $d$, respectively. Hence, $\CongrCAs{d}$ is the
congruence relation induced by $\LC{d}\CongrCAs{d}\RC{d}$.

We denote by CAs the set of $d$-comb associative operads where $d$ belongs
the set of strictly positive integers:
\[\text{CAs}=\left\{\CAs{d}\mid d\geq 1\right\}.\]
...
\begin{Proposition}\label{prop:morphisms_CAs_d}
  ...
  Let $d$ and $d'$ be two strictly positive integers. There exists at most
  one operads morphism $\varphi:\CAs{d'}\longrightarrow\CAs{d}$. This morphism
  exists if and only if $\LC{d'}\CongrCAs{d}\RC{d'}$ and it is surjective in this
  case. Moreover, $\varphi$ is injective if and only if $d=d'$, that is if and only
  if $\varphi$ is the identity morphism.
  
  \end{Proposition}

\begin{proof}
  The operad $\CAs{d'}$ is generated by one binary generator $t^{(d')}$, so that
  $\varphi$ is entirely determined by $\varphi\left(t^{(d')}\right)$. Moreover,
  $\varphi\left(t^{(d')}\right)$ has to be of arity $2$ in $\CAs{d}$, so that
  $\varphi\left(t^{(d')}\right)$ has to be equal to $t^{(d)}$. Hence, if $\varphi$
  exists, it is the unique operads morphism from $\CAs{d'}$ and $\CAs{d}$,
  and in this case, $t^{(d)}$ being in the image of $\varphi$, the latter is
  surjective. Moreover, $\varphi$ is well defined if and only if
  $\varphi\left(\LC{d'}\right)$ and $\varphi\left(\RC{d'}\right)$ are equal in
  $\CAs{d}$, that is if and only if $\LC{d'}\CongrCAs{d}\RC{d'}$. In particular,
  if $d'$ is strictly smaller than $d$, then there does not exist any morphism
  $\varphi:\CAs{d'}\longrightarrow\CAs{d}$. Moreover, if $d'$ is strictly smaller
  than $d$, the cardinality of $\CAs{d'}\left(d\right)$ is equal to the cardinality
  of $\CAs{d}(d)$ plus one. Hence, if there exists an injective morphism
  $\varphi:\CAs{d'}\longrightarrow\CAs{d}$, we must have $d=d'$, and in this case,
  $\varphi$ is the identity morphism.
\end{proof}

We define the binary relation $\preceq$ on CAs as follows: we have $\CAs{d}\preceq\CAs{d'}$
if and only if there exists a morphism $\varphi:\CAs{d'}\longrightarrow\CAs{d}$.

\begin{Theorem}\label{thm:poset_CAs}
...
\begin{proof}
  ...
  of $\CAs{d}$ for every strictly positive integer $d$. It is transitive since if
  there exist two morphismsk  $\varphi:\CAs{d}\longrightarrow\CAs{d'}$ and
  $\psi:\CAs{d''}\longrightarrow\CAs{d'}$, then $\psi\circ\varphi$ is a morphism from
  $\CAs{d''}$ to $\CAs{d}$. Now, let us assume that there exist morphisms
  $\varphi:\CAs{d'}\longrightarrow\CAs{d}$ and $\psi:\CAs{d}\longrightarrow\CAs{d'}$.
  In particular, $\psi\circ\varphi$ and $\varphi\circ\psi$ are endomorphisms of $\CAs{d'}$
  and $\CAs{d}$, respectively. From Proposition~\ref{prop:morphisms_CAs_d}, these two
  morphisms are identity morphisms, so that $\varphi$ and $\psi$ are injective. From
  Proposition~\ref{prop:morphisms_CAs_d}, $d$ and $d'$ are equal, which proves that
...
\begin{Lemma}\label{lem:left_rank_and_CongrCAs}
  Let $d$ be a strictly positive integer and let $t$ and $t'$ be two elements of $\Mag$. If
  $t\CongrCAs{d}t'$, then $\emph{lr}\left(t\right)-\emph{lr}\left(t'\right)$ is divisible by
  $d-1$.
\end{Lemma}

\begin{proof}
  Let $\Rew$ be the rewrite relation on $\Mag$ induced by the rewrite rule
  $\LC{d}\Rew\RC{d}$. In the rest of the proof we fix two elements $t$ and $t'$ of $\Mag$.

  First, we show that $t\Rew t'$ implies that $\text{lr}\left(t'\right)-\text{lr}\left(t\right)$
  is divisible by $d-1$ \textbf{TODO}.
  ...
  is divisible by $d-1$ from the first part of the proof, so that
  $\text{lr}\left(t'\right)-\text{lr}\left(t\right)$ is divisible by $d-1$.
  ...
  $\text{lr}\left(t_{i+1}\right)-\text{lr}\left(t_i\right)$ is divisible by $d-1$, so that
  $\text{lr}\left(t'\right)-\text{lr}\left(t\right)$ is divisible by $d-1$.
  
  The congruence relation $\CongrCAs{d}$ is the equivalence relation induced by $\Rew$, that
  is we have $t\CongrCAs{d}t'$ if and only if $t\overset{*}{\leftrightarrow}t'$. Hence, from the
  third part of the proof, $t\CongrCAs{d}t'$ implies that
  $\text{lr}\left(t'\right)-\text{lr}\left(t\right)$ is divisible by $d-1$.
  \end{proof}

\begin{Proposition} \label{prop:division_CAs}
  Let $d$ and $d'$ be two strictly positive integers. There exists a morphism
  $\varphi:\CAs{d'}\longrightarrow\CAs{d}$ if and only if $\left(d-1\right)\mid\left(d'-1\right)$.
\end{Proposition}
\begin{proof}
  From Proposition~\ref{prop:morphisms_CAs_d}, it is sufficient to show that $\LC{d'}\CongrCAs{d}\RC{d'}$
  if and only if $\left(d-1\right)\mid\left(d'-1\right)$. If $\LC{d'}\CongrCAs{d}\RC{d'}$, from
  Lemma~\ref{lem:left_rank_and_CongrCAs}, $\text{lr}\left(\LC{d'}\right)-\text{lr}\left(\RC{d'}\right)=d'-1$
  is divisible by $d-1$, which shows the direct implication. Conversely, if
  $\left(d-1\right)\mid\left(d'-1\right)$, the rewrite rule $\LC{d}\Rew\RC{d}$ induces the
  following sequence of rewriting steps:
  \[\textbf{Rewriting sequence from left comb of degree d' to right comb of degree d'}\]
  Hence, we have $\LC{d'}\CongrCAs{d}\RC{d'}$.
   \end{proof}
...
\begin{Theorem}\label{thm:lattice_CAs}
  ...
  \[\CAs{d}\wedge\CAs{d'}=\CAs{\text{gcd}\left(d-1,\ d'-1\right)}\
  \text{and}\ \CAs{d}\vee\CAs{d'}=\CAs{\text{lcm}\left(d-1,\ d'-1\right)}.\]
  \end{Theorem}
